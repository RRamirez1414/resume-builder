...
router.post(
  ...
  async (req, res) => {
    ...
    await ResumeInfo.deleteOne({ _id: req.params.resumeid })
    res.redirect(`/user/${loggedUser.username}/view-list`)
  }
)
...
/**
 * function to handle saving resume edits info to db
 */
async function updateResumeInfo(reqBody, resumeID) {
  ...
  try {
    ...
    if (
      typeof info.Skills != 'undefined' &&
      info.Skills != null &&
      Array.isArray(info.Skills) == true
    ) {
      resumeInfo.skills = ''
      ...
      typeof info.Skills != 'undefined' &&
      info.Skills != null &&
      Array.isArray(info.Skills) == false
    ) {
...
/**
 * function to handle saving experience info to db
 */
async function saveExperienceInfo(reqBody, resumeID) {
  ...
  if (
    ...
    Array.isArray(info.expName) == true &&
    info.expName.length != null &&
    info.expName.length > 1 &&
    info.expName.length != 0
  ) {
    ...
    typeof info.expName != 'undefined' &&
    Array.isArray(info.expName) == false
  ) {
    let experience = new Experiences({
      resumeInfo: resumeID,
...
/**
 * function to handle saving education info to db
 */
async function saveEducationInfo(reqBody, resumeID) {
  const info = reqBody
  console.log(Array.isArray(info.institution))
  if (
    typeof info.institution != 'undefined' &&
    Array.isArray(info.institution) == true &&
    info.institution.length != null &&
    info.institution.length > 1 &&
    info.institution.length != 0
  ) {
    ...
    typeof info.institution != 'undefined' &&
    Array.isArray(info.institution) == false
  ) {
    let education = new Educations({
      resumeInfo: resumeID,
...
/**
 * function to handle saving new resume info to db
 */
